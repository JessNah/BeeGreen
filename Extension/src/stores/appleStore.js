...
export function getAppleCart() {
    ...
    for(let i = 0; i < _cartItems.length; i++){
        ...
        const imageItem = _cartItems[i].getElementsByClassName("rs-iteminfo-image");
        let _item = {
            id: textItem[0].childNodes[0].innerText?.trim(),
            name: textItem[0].childNodes[0].innerText?.trim(),
            price: priceItem[0].childNodes[0].innerText.trim(),
            image: imageItem[0].childNodes[0].currentSrc
        }
        cart.push(_item);
    }
